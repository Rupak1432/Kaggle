...
if __name__ == '__main__':
    ...
    # Naive bayes
    gs_clf_mnb = GridSearchCV(twit_mnb_stemmed, parameters, n_jobs = -1)
    gs_clf_mnb = gs_clf_mnb.fit(x_train, y_train)
    print('gs_clf_mnb - best score',gs_clf_mnb.best_score_)
    print('gs_clf_mnb - best params',gs_clf_mnb.best_params_)
    predicted_mnb_gs = gs_clf_mnb.predict(x_test)
    print('naive bayes gs stemmed: ',np.mean(predicted_mnb_gs == y_test))

    #svm
    gs_clf_svm_stem = GridSearchCV(twit_svm_stemmed, parameters_svm, n_jobs=-1)
    gs_clf_svm_stem = gs_clf_svm_stem.fit(x_train, y_train)
    print('gs_clf_svm.best_score: ',gs_clf_svm_stem.best_score_)
    print('gs_clf_svm.best_params: ',gs_clf_svm_stem.best_params_)
    predicted_svm_gs = gs_clf_svm_stem.predict(x_test)
    print('svm gs stemmed: ',np.mean(predicted_svm_gs == y_test))
